...
func configureVRRP(services []Service, tmplFile string, configFile string) {

	for n, service := range services {
		log.Infof("-+= Service #%v", n)
		log.Infof(" |--= VRRP IP Address : %v", service.LoadBalancerIP)
	}
	...
	log.Infof("Ready to reload keepalived")

	out, err := exec.Command(config.reloadScript).CombinedOutput()
	if err != nil {
		log.Errorf("Error reloading keepalived: %v\n%s", err, out)
	} else {
...
func init() {
	...
	flag.StringVar(&config.tmplFile, "tmplFile", "vrrp.tmpl", "VRRP template file to load")
	flag.StringVar(&config.configFile, "configFile", "vrrp.conf", "VRRP Configuration file to write")
...
func main() {
	...
	configureVRRP(currentServices, config.tmplFile, config.configFile)
